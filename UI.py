...
class Label(UIItem):
	def __init__(self, label):
		UIItem.__init__(self, Vector2(100, 30))
		self.label = label
		self.update_surface()
	...
	def update_surface(self):
		...
		self.surface.blit(textsurface, (3, 0))
...
class Button(UIItem):
	...
	def update_surface(self):
		textsurface = self.font.render(self.label, False, (255, 255, 255))
		self.surface.fill((52, 152, 219))
		self.surface.blit(textsurface, (3, 0))
		return self.get_surface()

	def mouse_down(self, x, y):
		if self.is_mouse_in(x, y):
			self.action()

class InputField(UIItem):
	def __init__(self, value):
		UIItem.__init__(self, Vector2(200, 30))
		self.value = value
		self.update_surface()
...
class SettingsScreen(UIElement):
	...
	def save(self):
		for key in self.settings:
			self.settings[key] = self.input_fields[key].get_value()
	...
	def mouse_down(self, x, y):
		...
		if not self.keyboard.show:
			...
			self.save_button.mouse_down(x-200, y-200)
