...
class pyscope:
	...
	def __init__(self):
		...
		self.current_screen = self.inbox
	...
	def draw(self):
		...
		if True:
			...
			textsurface = self.font.render(str(int(self.fps)), False, (255, 255, 255))
			self.screen.blit(textsurface,( width - 60, height - 30))


		pygame.display.update()
		now = datetime.now()
		self.fps = 1000000. / (now - self.last_update).microseconds
		self.last_update = datetime.now()
